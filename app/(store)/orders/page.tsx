...
async function OrderPage() {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
            <div className="bg-white p-4 sm:p-8 rounded-2xl shadow-lg w-full max-w-4xl">
                <h1 className="text-4xl font-bold text-gray-900 tracking-tight mb-8">
                    My orders
                </h1>
                {orders.length === 0 ? (
                    <div className="text-center text-gray-50-600">
                        <p> You have no placed any orders yet.</p>
                    </div>
                ) : (
                    <div className="space-y-6 sm:space-x-0-y-8">
                        {orders.map((order) => (
                            <div
                                    ...
                                    <div className="space-y-3 sm:space-y-4">
                                        {order.products?.map((product) => (
                                            <div
                                                ...
                                                className="flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b last:border-b-0"
                                            >
                                                <div className="flex items-center gap-3 sm:gap-4">
                                                    {product.product?.image && (
                                                        <div className="relative h-14 w-14 sm:h-16 sm:w-16 flex-shrink-0 rounded-md overflow-hidden">
                                                            <Image
                                                                src={imageUrlFor(product.product.image).url()}
